...
int object_write(ObjectType type, const void *data, size_t len, ObjectID *id_out) {
    ...
    // Rename tmp file to final path atomicaly
...
int object_read(const ObjectID *id, ObjectType *type_out, void **data_out, size_t *len_out) {
    ...
    char type_str[32];
    size_t size;
    if (sscanf((char *)file_data, "%31s %zu", type_str, &size) != 2) {
        free(file_data);
        return -1;
    }
    ...
    int header_len = null_byte - (char *)file_data + 1;
    size_t actual_data_len = file_size - header_len;
    
    if (actual_data_len != size) {
        free(file_data);
        return -1;
    }
    
    void *out_buffer = malloc(actual_data_len);
    if (!out_buffer) {
        free(file_data);
        return -1;
    }
    memcpy(out_buffer, null_byte + 1, actual_data_len);
